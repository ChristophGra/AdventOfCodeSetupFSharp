...
let walls =
  Array2D.createBased 0 (miny - 200) (maxx + 1 + 2) (maxy - miny + 1 + 2 * 200) false
...
let rec flowSand (currentx,currenty) =
  try
    if walls[currentx,currenty] then
      false
    else if currentx = maxx + 1 then
      walls[currentx, currenty] <- true
      true
    else if not (walls[currentx + 1,currenty] ) then
