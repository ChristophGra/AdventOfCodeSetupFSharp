...
let parsed =
    ...
    |> fun (folder,_) -> getFolders folder
    |> Seq.where (fun x -> x.FolderSize <= 100000)
    |> Seq.sumBy (fun x -> x.FolderSize)
    //|> Seq.iter (fun x -> Console.WriteLine $"{x.CurrentFolder}: {x.FolderSize}")